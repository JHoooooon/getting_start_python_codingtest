...
2. 첫 라인을 사용하여 email 의 개수를 가져온다 -> email_count
3. 이후 라인들을 가져와 이메일을 배열화 시킨다
4. 결과 배열을 만든다
5. @ 기호 앞 문자의 . 을 무시한 이메일들을 만들고 결과 배열에 넣는다 
6. + 와 @ 사이의 문자를 무시하는 이메일들을 만들고 결과 배열에 넣는다
7. 대문자를 소문자로 변경한 이메일들을 만들고 결과 배열에 넣는다
9. 만든 SET 을 순회하여, 이메일을 KEY 로 하고 개수를 VALUE 로 하는
    DICTIONARY 를 만든다
10. DICTIONARY의 값을 프린트 한다
